...
using OffsetGenerator.Common.Templates;

namespace OffsetGenerator.App.Templates;

public class CppTemplate : BaseTemplate
{
    private static Dictionary<string, int> _typeSizeMap = new () 
    {
    ...
    private void GenerateSizes(NamespaceDefinition namespaceDefinition, int indent)
    {
        ...
        if (_typeSizeMap.TryGetValue(lastOffset.FoundField.FieldType, out var size))
            lastOffsetSize = size;
    ...
    {
        Logger.Log(LogType.Info, $"Generating namespace {namespaceDefinition.DumpNamespaceName}");
        ...
        if (namespaceDefinition.NamespaceDefinitions.Count > 0)
        {
            ...
            foreach (var namespaceDefinition in namespaceDefinition.NamespaceDefinitions)
                GenerateNamespace(namespaceDefinition, indent + 1, namespaceDefinition == namespaceDefinition.NamespaceDefinitions.Last());
        }
